...
"""
Taking 5 features as the input to the time series - open, high, low, close, Adjusted
close
"""
...
TimeseriesGenerator is used to automatically transform both univariate and multivariate
time series data into samples, ready to train deep learning models.
...
predictions.shape[0]
# The shape of output is 942 and not 1002 when compared with x_test
# since the first 60 days are used to determine the next day.

x_test[:, 1:][win_length:]
# Remove the first 60 values from the x_test and consider
# only the 942 values after the first 60 value
...
# Get the last 942 rows to compare the actual and the predicted opening price
